...
import { archiveExperiment, deleteExperiment, duplicateExperiment, getExperiment, pauseExperiment, resumeExperiment, saveExperiment, todayKey, tomorrowKey } from "@/lib/storage";
import { baselineEntries, baselineProgress, canFinishExperiment, currentABVariant, effectiveExperimentDay, experimentMetrics, experimentProgress } from "@/lib/results";
import { Experiment, ExperimentDecision, ExperimentEntry, ExperimentVariant } from "@/lib/types";
...
export default function ExperimentDetailPage() {
  ...
  function syncForm(found: Experiment) {
    const phase = found.phase ?? "test";
    const variant = (found.mode ?? "single") === "ab" ? currentABVariant(found) : undefined;
    const today = found.entries.find((entry) => entry.date === todayKey() && (entry.phase ?? "test") === phase && (!variant || entry.variant === variant));
  ...
  function submit(event: FormEvent) {
    ...
    if (phase === "baseline") {
      const entry: ExperimentEntry = { date: todayKey(), completed: true, value, note: note.trim() || undefined, phase: "baseline" };
      const exists = current.entries.some((item) => item.date === entry.date && item.phase === "baseline");
      const nextEntries = exists ? current.entries.map((item) => item.date === entry.date && item.phase === "baseline" ? entry : item) : [...current.entries, entry];
      const observed = nextEntries.filter((item) => item.phase === "baseline");
      const required = Math.max(1, current.baselineDays ?? 2);
      const baselineValue = observed.reduce((sum, item) => sum + item.value, 0) / observed.length;
      const next: Experiment = observed.length >= required
        ? { ...current, entries: nextEntries, baseline: baselineValue, phase: "test", baselineCompletedDate: todayKey(), startDate: tomorrowKey() }
        : { ...current, entries: nextEntries };
      saveExperiment(next); setExperiment(next); setEditingDate(null); setEditingVariant(undefined); syncForm(next); setSaved(true); window.setTimeout(() => setSaved(false), 1800); return;
    }
    ...
    const next: Experiment = { ...current, entries: exists ? current.entries.map((item) => sameEntry(item, entry) ? entry : item) : [...current.entries, entry] };
    saveExperiment(next); setExperiment(next); setEditingDate(null); setEditingVariant(undefined); syncForm(next); setSaved(true); window.setTimeout(() => setSaved(false), 1800);
  }

  function startEdit(entry: ExperimentEntry) {
    if (current.status !== "active") return;
    setEditingDate(entry.date); setEditingVariant(entry.variant); setValue(entry.value); setCompleted(entry.completed); setNote(entry.note ?? ""); window.scrollTo({ top: 0, behavior: "smooth" });
  }

  function removeEntry(entry: ExperimentEntry) {
    if (!window.confirm("¿Eliminar este registro?")) return;
    const next = { ...current, entries: current.entries.filter((item) => !sameEntry(item, entry)) };
    saveExperiment(next); setExperiment(next); if (editingDate === entry.date && editingVariant === entry.variant) { setEditingDate(null); setEditingVariant(undefined); syncForm(next); }
  }
  ...
  function finishExperiment() {
    ...
    saveExperiment(next); setExperiment(next);
  }
  ...
  return (
    <main className="product-page detail-page ui-pass-page method-detail-page">
      ...
      {waitingForTest && current.status === "active" && <section className="status-banner baseline-ready"><div><small>BASELINE COMPLETO</small><strong>Tu punto de partida es {current.baseline.toFixed(1)}{current.metricUnit}.</strong><span>Mañana empieza la intervención. Hoy ya has terminado.</span></div><span className="baseline-live">✓<small>listo</small></span></section>}
      ...
      <div className="detail-grid detail-grid-v2">
        <section className="checkin-card checkin-card-v2">
          <p className="eyebrow">{editingDate ? `EDITANDO ${editingDate}` : baselineMode ? "OBSERVACIÓN DE HOY" : "CHECK-IN DE HOY"}</p><h2>{editingDate ? "Corrige este registro" : baselineMode ? "¿Cómo estás hoy?" : "¿Cómo ha ido?"}</h2>
          {current.status === "active" && !waitingForTest ? <form onSubmit={submit}>
            {!baselineMode && <div className="binary-row"><button type="button" className={completed ? "binary active" : "binary"} onClick={() => setCompleted(true)}>{mode === "ab" ? `✓ Sí, respeté ${editingVariant ?? variant}` : "✓ Sí, lo hice"}</button><button type="button" className={!completed ? "binary active" : "binary"} onClick={() => setCompleted(false)}>No hoy</button></div>}
            {baselineMode && <p className="baseline-instruction">Registra la señal sin aplicar todavía el cambio que quieres probar.</p>}
            ...
            <div className="checkin-actions">{editingDate && <button className="button button-secondary" type="button" onClick={() => { setEditingDate(null); setEditingVariant(undefined); syncForm(current); }}>Cancelar</button>}<button className="button button-primary" type="submit">{saved ? "✓ Guardado" : editingDate ? "Guardar cambios" : baselineMode ? "Guardar observación" : "Guardar check-in"}</button></div>
