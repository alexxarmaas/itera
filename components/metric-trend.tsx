...
import { testEntries } from "@/lib/results";

export default function MetricTrend({ experiment }: { experiment: Experiment }) {
  const entries = testEntries(experiment).sort((a, b) => a.date.localeCompare(b.date));
  ...
  return (
    <div className="metric-trend">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`Tendencia de ${experiment.metricLabel}`}>
        {!isAB && <line className="trend-baseline" x1={pad} x2={width - pad} y1={baselineY} y2={baselineY} />}
        <polyline className="trend-line" points={points} />
        {entries.map((entry, index) => <circle className={`trend-point ${entry.variant ? `variant-${entry.variant.toLowerCase()}` : ""}`} cx={x(index)} cy={y(entry.value)} r="3.5" key={`${entry.date}-${entry.variant ?? "single"}-${index}`} />)}
      </svg>
      <div className="trend-foot"><span>{isAB ? `${experiment.variantA ?? "A"} vs ${experiment.variantB ?? "B"}` : `Baseline ${experiment.baseline.toFixed(1)}${experiment.metricUnit}`}</span><span>{entries.length} registros</span></div>
    </div>
  );
}
